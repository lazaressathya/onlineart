		...
		<div class="wrap-login100 p-l-55 p-r-55 p-t-40 p-b-30 ">
			...
			<div class="container-login100-form-btn">
					<div class="row mr-5 mb-4 ">
						<h1 class="font-weight-bold float-center ml-5" style="color:#C6A16E">Online Art Gallery</h1>
					</div>
				</div>
				...
				<div class="text-center">
					<h6><a href="logout.php" class="txt2 hov1">Cancel</a></h6>
				</div>
			</form>
		</div>
    </div>
...
if(isset($_POST['go'])) 
{
    ...
    $user=$_SESSION['user_session'];
                ...
                foreach($rows as $row)
                {
                               ...
                               if($result = $mongo->executeBulkWrite('pack.user', $bulk))
                               {
                                    //$_SESSION['user_session']=$user;
                                    echo "<script>if(confirm('Password changed, try loging in.')){document.location.href='login.php'};</script>";
              ...
              //echo $email; 
               ...
               $qry=new MongoDB\Driver\Query([]);
                ...
                foreach($rows as $row)
                {
                                ...
                                if($result = $mongo->executeBulkWrite('VenueBooking.admin', $bulk))
                                {
                                    //$_SESSION['user_session']=$user;
                                    
                                    echo "<script>if(confirm('Password changed, try loging in. ')){document.location.href='logout.php'};</script>";
